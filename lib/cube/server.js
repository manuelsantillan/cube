var util = require("util"),
    ...
    tcp_transport=require("./tcp-transport");
...
module.exports = function(options) {
  ...
  server.start = function() {
    ...
    database.open(options, function (error, db) {
      ...
      if(endpoints.tcp){
          util.log("Starting analitix tcp server on port "+options["udp-port"]);
          tcp_transport.listen(options["udp-port"], function(host){
              tcp_transport.onmsg(host, function(msg, host){//on message process
                 endpoints.tcp(msg, checkErrors); //same as udp, we ignore the response
              });
           });
      }
      if (endpoints.udp) {
        ...
        udp.on("message", function(message) {
          endpoints.udp(JSON.parse(message.toString("utf8")), checkErrors);
        });
...
function checkErrors(result) {
  // Responses for UDP are ignored; there's nowhere for them to go!
    if(result){
        if(result.error){
            console.log(result.error, new Error().stack);
        }
    }
