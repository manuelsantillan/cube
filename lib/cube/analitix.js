...
exports.getter = function(db) {
    ...
    function getter(request, callback) {
        if(util.isArray(request.channel)){
            for(var i = 0; i < request.channel.length; i++) {

                (function(i) {
                    getterChannel(request,request.channel[i], callback);
                })(i);

            }
        }else{
            getterChannel(request,request.channel, callback);
        }
