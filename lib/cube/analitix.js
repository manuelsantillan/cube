...
exports.putter = function(db) {
    channelCfgCache=require("./analitix/channelsCfgCache").initCache(db);
    ...
    function getter(request, callback) {
        ...
        try {
            if(!request.expression){
                ...
                var channelEventType=  request.channel.replace(/\//gi,"_").replace(/\'/gi, '');
                newExpression=util.format(newExpression, channelEventType, request.channel);
                ...
                collection(expression.type).events.aggregate(
                    ...
                    function(err, result){
                        ...
                        cursor.each(function(error, event) {
                            ...
                            var timeSeriesEvt=timeSeries.formatSeries(collection, event.d);
