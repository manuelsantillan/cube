...
var mongodb = require("mongodb"),
    ...
    timeSeries=require("./analitix_time_series"),
    util = require("util");
...
exports.putter = function(db) {
    ...
    function putter(request, callback) {
        var fullEvent={type: request.type, data: request.data};
        if(request.time) fullEvent.time=request.time;
        if(request.id) fullEvent.id=request.id;
        if(timeSeries.isTimeSeries(fullEvent)){
            fullEvent=timeSeries.formatEvent(fullEvent);
        }

        var time = "time" in fullEvent ? new Date(request.time) : new Date(),
            type = fullEvent.type;
        ...
        var errors=timeSeries.eventHasErrors(fullEvent);
        //add sampleCount
        fullEvent.data.header.sampleCount=fullEvent.data.samples.length;

        // If an id is specified, promote it to Mongo's primary key.
        var event = {t: time, d: fullEvent.data};

        if ("id" in fullEvent) event._id = fullEvent.id;
        if(errors) {
      ...
      console.log("fields", fields);
        ...
        collection(expression.type).events.count(filter, function(err, n){
          ...
          collection(expression.type).events.aggregate(
              {$match: filter},
              {$group: { _id: "$d.header.id", sampleCount: { $sum: "$d.header.sampleCount" } }},
              function(err, result){
        ...
        (function poll() {
          query(function(event) {
            ...
            if (event) {
              ...
              // Sergio added. when number of events is greater than bachsize then
              // only is returned this element
              // TODO problems with before inserts  in mongodb with date before.
              // TODO Add flag for deactivate this functionality
              if(event.data.header && event.data.samples){
                    sampleRate = 1/event.data.header.sampleRate;
                    var numberOfSamples = event.data.samples.length;
                    var time =  new Date(event.data.header.timeStamp);
                    var increment =  sampleRate*numberOfSamples*1000;
                    time.setMilliseconds(time.getMilliseconds()+increment);
                streamsBySource[expression.source].lastEvent = event.time;
              }
              // end changes
            }
            ...
            // the time range, and set a timeout for the next poll.
              ...
              if (!streams.active.length) {
                ...
                // Sergio added. when number of events is greater than bach size then
                // only is returned this element
