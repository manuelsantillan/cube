...
exports.register = function(db, endpoints) {
  var putter = require("./event").putter(db),
      poster = post(putter),
      analitix_putter=require("./analitix").putter(db);

  //
  endpoints.ws.push(
    endpoint("/1.0/event/put", putter),
    endpoint("1.0/analitix/put", analitix_putter)
  );
  ...
  endpoints.http.push(
    ...
    endpoint("POST", "1.0/analitix/put", analitix_putter)

  );

  //
  endpoints.udp = analitix_putter;
  endpoints.tcp=analitix_putter;
};
