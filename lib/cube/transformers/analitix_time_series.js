...
exports.formatEvent=function(timeSeries){
    ...
    if((timeSeries.body && timeSeries.body.metadata && timeSeries.body.metadata.timestamp)){
        event.t=new Date(timeSeries.body.metadata.timestamp);
        event.d.body.metadata.timestamp = new Date(timeSeries.body.metadata.timestamp);
        event.d.b.m.sC=timeSeries.body.y.length;
    }

    if(timeSeries.b && timeSeries.b.m && timeSeries.b.m.ts){
        event.t=new Date(timeSeries.b.m.ts);
        event.d.b.m.ts = new Date(timeSeries.b.m.ts);
        event.d.b.m.sC=timeSeries.b.y.length;
    }

    console.log('Count ', event.d.h.count);
    if(event.d.b.m.id=='1_1_6'){
        console.log('data 1_1_6 ', event.d.b.y);
    }
    return event;
...
exports.enrichEvent=function(event){
    ...
    if (channelCfg){
        event.body.metadata.name = channelCfg.identification.name;
        event.body.metadata.shortDesc = channelCfg.identification.shortDesc;
        event.body.metadata.unitOfMeasurement =  channelCfg.identification.unitOfMeasurement;
        event.body.metadata.channelType =  channelCfg.identification.channelType;
        event.body.metadata.channelNumber =  channelCfg.identification.channelNumber;
